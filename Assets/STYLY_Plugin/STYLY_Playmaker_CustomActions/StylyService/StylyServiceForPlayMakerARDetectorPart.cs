...
namespace STYLY
{
    ...
    public partial class StylyServiceForPlayMaker
    {
        ...
        /// <param name="mapFile">Immersalのマップ作成アプリで作成したbytes形式のファイル</param>
        /// <param name="onFinished">終了イベント</param>
        public void ImmersalInit(TextAsset mapFile, Action<Exception> onFinished)
        {
            if (arDetectormpl != null)
            {
                arDetectormpl.ImmersalInit(mapFile, onFinished);
            }
            else
            {
                //未実装ならエラー通知
                Debug.Log("ImmersalInit called with map file: <" + mapFile?.name + ">");
                onFinished(new Exception("StylyServiceForPlayMaker implementation not available."));
            }
        }
        ...
        /// </summary>
        /// <param name="onFinished">終了イベント</param>
        public void ImmersalLocalize(Action<Exception> onFinished)
        {
            if (arDetectormpl != null)
            {
                arDetectormpl.ImmersalLocalize(onFinished);
            }
            else
            {
                //未実装ならエラー通知
                Debug.Log("ImmersalLocalize called.");
                onFinished(new Exception("StylyServiceForPlayMaker implementation not available."));
            }
        }
    }
